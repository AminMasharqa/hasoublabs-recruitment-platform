...
These have been resolved in the requirements. This table documents the decisions made and updates applied.

| # | Issue | Resolution |
| --- | --- | --- |
| O-1 | Glossary defines **CV** as "PDF format", but R5 AC1 accepts "a valid PDF or DOCX". | Updated glossary to include both PDF and DOCX formats. |
...
| O-4 | R5 AC11 is marked `<todo: candidate and admin can't change active version>` — the AC text says they can, the TODO says they cannot. | TODO resolved: explicit active version designation is not allowed. A new upload automatically becomes the `active` version for that variant. |
